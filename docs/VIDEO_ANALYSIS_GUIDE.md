...
```bash
# 获取完整结果
curl http://localhost:8001/video/tasks/{task_id}

# 仅获取进度
curl http://localhost:8001/video/tasks/{task_id}/progress

# 列出所有任务
curl http://localhost:8001/video/tasks
```
...
### GET /video/tasks/{task_id}/progress
    ...
    async with httpx.AsyncClient() as client:
        ...
        for path in video_paths:
            response = await client.post(
                "http://localhost:8001/video/analyze/async",
                json={
        ...
        for task_id in tasks:
            while True:
                status = await client.get(f"http://localhost:8001/video/tasks/{task_id}")
