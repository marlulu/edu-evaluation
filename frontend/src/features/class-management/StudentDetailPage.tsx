...
import { listWorkTasks } from '../work-analysis/api';
import {
...
export function StudentDetailPage() {
  ...
  const handleAddWork = async (taskId: string) => {
    if (!studentId) return;
    try {
      await addWorkToStudent(studentId, taskId);
  ...
  const handleRemoveWork = async (taskId: string) => {
    if (!studentId) return;
    try {
      await removeWorkFromStudent(studentId, taskId);
    ...
    <div style={{ padding: 24 }}>
      ...
      <Card size="small" style={{ marginBottom: 16 }}>
        <Space>
          ...
          <UserOutlined />
          <Text strong style={{ fontSize: 16 }}>{student?.studentName || '加载中...'}</Text>
          {student?.studentNumber && (
            <Tag>{student.studentNumber}</Tag>
          )}
        </Space>
      </Card>
