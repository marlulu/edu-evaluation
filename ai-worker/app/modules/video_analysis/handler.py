...
class VideoAnalyzer:
    ...
    def __init__(
        ...
        # 临时目录
        self.temp_dir = Path(tempfile.mkdtemp(prefix="video_analysis_"))
    ...
    async def analyze(
        ...
        def update_progress(status: VideoTaskStatus, progress: float, detail: str = ""):
            if progress_callback:
                progress_callback(VideoAnalysisProgress(
                    task_id=task_id,
                    status=status,
                    progress=progress,
            ...
            if request.options.extract_keyframes:
                ...
                for kf in keyframes:
                    if kf.path and Path(kf.path).exists():
                        # 尝试上传到 MinIO
                        url = self._upload_to_minio(Path(kf.path), f"video-keyframes/{task_id}/frame_{kf.index:04d}.jpg")
                        if url:
...
class VideoAnalysisManager:
    ...
    def __init__(self):
        self.tasks: dict[str, VideoAnalysisResult] = {}
    ...
    async def submit_task(self, request: VideoAnalysisRequest) -> str:
        """提交分析任务"""
        task_id = request.task_id

        self.tasks[task_id] = VideoAnalysisResult(
            task_id=task_id,
            file_name=request.file_name,
    ...
    async def _execute_task(self, request: VideoAnalysisRequest):
        """执行分析任务"""
        task_id = request.task_id
        analyzer = self.get_analyzer()

        def progress_callback(progress: VideoAnalysisProgress):
    ...
    def get_task(self, task_id: str) -> VideoAnalysisResult | None:
        """获取任务状态"""
...
# 全局实例
video_manager = VideoAnalysisManager()
